...
void Game::initMonsters()
{
	...
	for (int r = 0; r < ROWS; r++)
		for (int c = 0; c < COLUMNS; c++)
		{
			...
			if (levels[current_level].getValue(pos) == ATTACKER_START)
			{
				monsters[next_monster] = new Attacker(pos);
				cout << "created attacker r: " << r << " c: " << c << endl;

				next_monster++;
			}
			/*else if (levels[current_level].getValue(pos) == MONSTER_START)
			{
			...
			else if (levels[current_level].getValue(pos) == DRONE_START)
			{
				...
				cout << "created drone r: " << r << " c: " << c << endl;

				next_monster++;
			}
			else if (levels[current_level].getValue(pos) == SENTRY_START)
			{
				monsters[next_monster] = new Sentry(pos);
				cout << "created sentry r: " << r << " c: " << c << endl;
